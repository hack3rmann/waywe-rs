...
pub struct VideoPipelineNv12 {
    screen_size: UVec2,
    video_planes: [wgpu::Texture; 2],
    vertex_buffer: wgpu::Buffer,
    bind_group: wgpu::BindGroup,
    pipeline: wgpu::RenderPipeline,
}

impl VideoPipelineNv12 {
    pub fn new(
        device: &wgpu::Device,
        surface_format: wgpu::TextureFormat,
        video_size: UVec2,
        ...
        let fragment_shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: None,
            source: wgpu::ShaderSource::Glsl {
                shader: include_str!("shaders/video-nv12.glsl").into(),
                stage: wgpu::naga::ShaderStage::Fragment,
                defines: Default::default(),
            },
        });
        ...
        let video_texture_uv_plane = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("video"),
            size: wgpu::Extent3d {
                width: video_size.x / 2,
                height: video_size.y / 2,
        ...
        let video_texuture_view_y_plane = video_texture_y_plane.create_view(&Default::default());
        let video_texuture_view_uv_plane = video_texture_uv_plane.create_view(&Default::default());
        let sampler = device.create_sampler(&wgpu::SamplerDescriptor {
            mag_filter: wgpu::FilterMode::Linear,
            ..Default::default()
        });
        ...
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry {
                    binding: 1,
                    resource: wgpu::BindingResource::TextureView(&video_texuture_view_uv_plane),
                },
                wgpu::BindGroupEntry {
                    binding: 2,
                    resource: wgpu::BindingResource::Sampler(&sampler),
    ...
    pub fn write_video_frame(&self, queue: &wgpu::Queue, data_planes: [&[u8]; 2]) {
        for ((texture, data), i) in self.video_planes.iter().zip(data_planes).zip(1..) {
            queue.write_texture(
                wgpu::TexelCopyTextureInfo {
                    texture,
                    mip_level: 0,
                ...
                wgpu::TexelCopyBufferLayout {
                    offset: 0,
                    bytes_per_row: Some(i * texture.width()),
                    rows_per_image: None,
                },
                texture.size(),
            );
