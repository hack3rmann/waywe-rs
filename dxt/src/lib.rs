...
fn get_color_palette(data: u32, format: DxtFormat) -> [u32; 4] {
    ...
    let r0 = (((color0 >> 11) & 31) * 255 / 31) as u8;
    let g0 = (((color0 >> 5) & 63) * 255 / 63) as u8;
    let b0 = (((color0 & 31) * 255) / 31) as u8;

    let r1 = (((color1 >> 11) & 31) * 255 / 31) as u8;
    let g1 = (((color1 >> 5) & 63) * 255 / 63) as u8;
    let b1 = ((color1 & 31) * 255 / 31) as u8;

    // NOTE(ArnoDarkrose): maybe make this from_be_bytes
    let mut palette: [u32; 4] = [
        u32::from_le_bytes([r0, g0, b0, 255]),
        u32::from_le_bytes([r1, g1, b1, 255]),
        0,
        0,
    ];
...
fn decompress_dxt_5_block(
    ...
    let alpha0 = first_num_bytes_view[0];
    let alpha1 = first_num_bytes_view[1];

    let mut alpha_palette = [alpha0, alpha1, 0, 0, 0, 0, 0, 0];

    if alpha0 > alpha1 {
        for i in 0u8..6 {
            alpha_palette[(i + 2) as usize] = ((6 - i) * alpha0 + (i + 1) * alpha1) / 7;
        }
    } else {
        for i in 0_u8..4 {
            alpha_palette[(i + 2) as usize] = ((4 - i) * alpha0 + (i + 1) * alpha1) / 5;
        }
    ...
    for i in 0..4 {
        for j in 0..4 {
            if bits < 3 {
                let idx = 2 + (i * 4 + j) * 3 / 8;
                if idx < 8 {
                    cur_byte |= first_num_bytes_view[idx] << bits;
                } else {
                    cur_byte |= second_num_bytes_view[idx % 8] << bits;
                }
                bits += 8;
            }
