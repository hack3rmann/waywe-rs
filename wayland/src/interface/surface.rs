...
pub mod request {
    ...
    impl<'s> Request<'s> for Attach {
            ...
            's: 'm,
        {
            WlMessage::builder(buf)
                .opcode(Self::CODE)
                .maybe_object(self.buffer.map(|h| storage.get_proxy(h).unwrap()))
                .int(self.x)
                .int(self.x)
                .build()
        }
    ...
    impl<'s> Request<'s> for SetOpaqueRegion {
            ...
            's: 'm,
        {
            WlMessage::builder(buf)
                .opcode(Self::CODE)
                .maybe_object(self.region.map(|h| storage.get_proxy(h).unwrap()))
                .build()
        }
    }
