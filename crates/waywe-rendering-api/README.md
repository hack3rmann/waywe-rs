...
The daemon loads scene wallpapers from `.ww` packages. Each package is a
gzip-compressed tar archive containing `wallpaper.so` (the renderer dylib) and an
optional `assets/` tree. The daemon unpacks the archive, loads the library, and passes
the package root as `working_directory` in `OpaqueRendererDesc`. Each renderer owns
its own `wgpu` device, receives a shared Vulkan render target from the daemon, draws
into it, and the daemon copies the result to the Wayland wallpaper surface each frame.

## Architecture

```text
waywe-daemon                              renderer (wallpaper.so)
     │                                           │
...
`OpaqueRendererDesc` contains `config: WallpaperConfig` and `working_directory` -- the
unpacked package root where `assets/` lives.
...
Put any runtime resources in `assets/` next to `Cargo.toml`, then build a `.ww`
package:

```shell
waywe package build --path path/to/my-renderer
waywe show target/release/my-renderer.ww
```

The CLI detects `.ww` archives and sends a `SetScene` command to the daemon, which
unpacks the package and loads `wallpaper.so` from it.
