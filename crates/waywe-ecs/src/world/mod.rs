...
use crate::{
    ...
    lifecycle::{ADD, ComponentHooks, DESPAWN, INSERT, REMOVE, REPLACE, RemovedComponentMessages},
...
impl World {
    ...
    pub fn register_component_hooks_by_id(
        ...
        assert!(
            ...
            "Components hooks cannot be modified if the component already exists in an archetype, use register_component if the component with id {id:?} may already be in use"
        );
            ...
            if let Some(first_location) = self.entities().get(first_entity) {
                ...
                for (entity, bundle) in batch_iter {
                    if let Some(location) = cache.inserter.entities().get(entity) {
                        ...
                        panic!(
                            "error[B0003]: Could not insert a bundle (of type `{}`) for entity {entity}, which {}. See: https://bevy.org/learn/errors/b0003",
                ...
                panic!(
                    "error[B0003]: Could not insert a bundle (of type `{}`) for entity {first_entity}, which {}. See: https://bevy.org/learn/errors/b0003",
                    DebugName::type_name::<B>(),
                    self.entities
                        .entity_does_not_exist_error_details(first_entity)
                );
            }
        }
    }
