...
fn normalize_binop<'cx>(o: BinOp, x: Nir<'cx>, y: Nir<'cx>) -> Ret<'cx> {
    ...
    match (o, x.kind(), y.kind()) {
        ...
        (NaturalPlus, Num(Natural(x)), Num(Natural(y))) => ret_kind(Num(Natural(x + y))),
        ...
        (NaturalTimes, Num(Natural(x)), Num(Natural(y))) => ret_kind(Num(Natural(x * y))),
        ...
        (TextAppend, NirKind::TextLit(x), _) => {
            ret_kind(NirKind::TextLit(x.concat(&TextLit::interpolate(y.clone()))))
        }
        (TextAppend, _, NirKind::TextLit(y)) => {
            ret_kind(NirKind::TextLit(TextLit::interpolate(x.clone()).concat(y)))
        }

        (RightBiasedRecordMerge, _, RecordLit(kvs)) if kvs.is_empty() => ret_nir(x),
        (RightBiasedRecordMerge, RecordLit(kvs), _) if kvs.is_empty() => ret_nir(y),
        ...
        (RecursiveRecordMerge, _, RecordLit(kvs)) if kvs.is_empty() => ret_nir(x),
        (RecursiveRecordMerge, RecordLit(kvs), _) if kvs.is_empty() => ret_nir(y),
...
fn normalize_field<'cx>(v: &Nir<'cx>, field: &Label) -> Ret<'cx> {
    ...
    match v.kind() {
        ...
        UnionType(kts) => ret_kind(UnionConstructor(field.clone(), kts.clone())),
            ...
            (RecordLit(kvs), _) => match kvs.get(field) {
                Some(r) => ret_op(Field(
                    Nir::from_kind(Op(BinOp(
                        ...
                        Nir::from_kind(RecordLit(once((field.clone(), r.clone())).collect())),
                        y.clone(),
                    ))),
                    field.clone(),
                )),
...
pub fn normalize_operation<'cx>(opkind: OpKind<Nir<'cx>>) -> Ret<'cx> {
    ...
    use NirKind::{
        EmptyListLit, EmptyOptionalLit, NEListLit, NEOptionalLit, Num, Op, RecordLit, RecordType,
        UnionConstructor, UnionLit,
    };
    use NumKind::Bool;
    use OpKind::*;
    ...
    match opkind {
        ...
        ToMap(v, annot) => match v.kind() {
            RecordLit(kvs) if kvs.is_empty() => match annot.as_ref().map(|v| v.kind()) {
        ...
        Projection(_, ls) if ls.is_empty() => ret_kind(RecordLit(HashMap::new())),
        Projection(v, ls) => match v.kind() {
            ...
            Op(Projection(v2, _)) => normalize_operation(Projection(v2.clone(), ls)),
        ...
        ProjectionByExpr(v, t) => match t.kind() {
            RecordType(kts) => normalize_operation(Projection(v, kts.keys().cloned().collect())),
