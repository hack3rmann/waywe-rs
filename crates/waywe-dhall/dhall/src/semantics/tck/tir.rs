...
impl<'cx> Type<'cx> {
    ...
    pub fn new_infer_universe(env: &TyEnv<'cx>, val: Nir<'cx>) -> Result<Self, TypeError> {
        ...
        let u = match c {
            ...
            None => unreachable!("internal type error: this is not a type: {:?}", val),
        };
        Ok(Type::new(val, u))
    }
    pub fn from_const(c: Const) -> Self {
...
impl<'cx, 'hir> Tir<'cx, 'hir> {
    ...
    pub fn ensure_is_type(&self, env: &TyEnv<'cx>) -> Result<(), TypeError> {
        if self.ty().as_const().is_none() {
            return mkerr(
                ...
                .span_err(
                    self.span(),
                    format!("this has type: `{}`", self.ty().to_expr_tyenv(env)),
                )
                .help(format!(
    ...
    /// Evaluate to a Type.
    pub fn eval_to_type(&self, env: &TyEnv<'cx>) -> Result<Type<'cx>, TypeError> {
