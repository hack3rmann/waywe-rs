...
impl<'cx> ImportEnv<'cx> {
    ...
    pub fn get_from_mem_cache(&self, location: &ImportLocation) -> Option<ImportResultId<'cx>> {
        Some(*self.mem_cache.get(location)?)
    }

    pub fn get_from_disk_cache(&self, hash: &Option<Hash>) -> Option<Typed<'cx>> {
    ...
    pub fn write_to_mem_cache(&mut self, location: ImportLocation, result: ImportResultId<'cx>) {
