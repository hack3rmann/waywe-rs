...
impl<'cx> Hir<'cx> {
    ...
    /// Typecheck the Hir.
    pub fn typecheck<'hir>(&'hir self, env: &TyEnv<'cx>) -> Result<Tir<'cx, 'hir>, TypeError> {
        type_with(env, self, None)
    }
    pub fn typecheck_noenv<'hir>(&'hir self, cx: Ctxt<'cx>) -> Result<Tir<'cx, 'hir>, TypeError> {
        typecheck(cx, self)
    }
...
fn hir_to_expr<'cx>(cx: Ctxt<'cx>, hir: &Hir<'cx>, opts: ToExprOptions, env: &mut NameEnv) -> Expr {
    let kind = match hir.kind() {
        ...
        HirKind::Expr(e) => {
            ...
            match e {
                ExprKind::Lam(_, t, e) if opts.alpha => ExprKind::Lam("_".into(), t, e),
                ExprKind::Pi(_, t, e) if opts.alpha => ExprKind::Pi("_".into(), t, e),
                e => e,
            }
        }
    };
