...
impl ImportLocationKind {
    fn chain_local(&self, prefix: FilePrefix, path: &FilePath) -> Result<Self, Error> {
        Ok(match self {
            ImportLocationKind::Local(..)
            | ImportLocationKind::Env(..)
            | ImportLocationKind::Missing => {
                let dir = match self {
                    ImportLocationKind::Local(path) => path.parent().unwrap().to_owned(),
                    ImportLocationKind::Env(..) | ImportLocationKind::Missing => {
                        std::env::current_dir()?
                    }
                    _ => unreachable!(),
                };
                ...
                let path = Some(prefix.to_string()).into_iter().chain(path).collect();
    ...
    fn to_location(&self) -> Expr {
        let (field_name, arg) = match self {
            ImportLocationKind::Local(path) => ("Local", Some(path.to_string_lossy().into_owned())),
            ImportLocationKind::Remote(url) => ("Remote", Some(url.to_string())),
            ImportLocationKind::Env(name) => ("Environment", Some(name.clone())),
            ImportLocationKind::Missing => ("Missing", None),
            ImportLocationKind::NoImport => unreachable!(),
        };

        let asloc_ty = make_aslocation_uniontype();
        let expr = mkexpr(ExprKind::Op(OpKind::Field(asloc_ty, field_name.into())));
        match arg {
...
impl ImportLocation {
    ...
    fn chain(&self, import: &Import) -> Result<ImportLocation, Error> {
        ...
        let kind = match &import.location {
            ImportTarget::Local(prefix, path) => self.kind.chain_local(*prefix, path)?,
    ...
    fn fetch<'cx>(&self, env: &mut ImportEnv<'cx>, span: Span) -> Result<Typed<'cx>, Error> {
        let cx = env.cx();
        let typed = match self.mode {
            ...
            ImportMode::RawText => {
                ...
                Typed {
                    hir: Hir::new(HirKind::Expr(ExprKind::TextLit(text.into())), span),
...
pub fn check_hash<'cx>(
    ...
    if let (ImportMode::Code, Some(Hash::SHA256(hash))) = (import.import.mode, &import.import.hash)
    {
        ...
        if hash[..] != actual_hash[..] {
            mkerr(
                ErrorBuilder::new("hash mismatch")
                    ...
                    .note(format!("Found    sha256:{}", hex::encode(actual_hash)))
                    .format(),
            )?
        }
    }
...
fn fetch_import<'cx>(
    ...
    let res_id = if let Some(res_id) = env.get_from_mem_cache(&location) {
        ...
        // current one.
        let res =
            env.with_cycle_detection(location.clone(), |env| location.fetch(env, span.clone()));
        let typed = match res {
            Ok(typed) => typed,
            Err(e) => mkerr(
...
fn traverse_accumulate<'cx>(
    ...
    let kind = match expr.kind() {
        ...
        ExprKind::Op(OpKind::BinOp(BinOp::ImportAlt, l, r)) => {
            ...
            let l = traverse_accumulate(env, name_env, &mut imports_l, base_location, l);
            let mut imports_r = Vec::new();
            let r = traverse_accumulate(env, name_env, &mut imports_r, base_location, r);
            let alt = cx.push_import_alternative(imports_l.into(), imports_r.into());
        ...
        kind => {
            let kind = kind.map_ref_maybe_binder(|l, e| {
                ...
                let hir = traverse_accumulate(env, name_env, nodes, base_location, e);
                if l.is_some() {
                    name_env.remove_mut();
                }
                hir
            ...
            match kind {
                ExprKind::Import(import) => {
                    ...
                    let import_id = cx.push_import(base_location.clone(), import, expr.span());
...
fn resolve_nodes<'cx>(env: &mut ImportEnv<'cx>, nodes: &[ImportNode<'cx>]) -> Result<(), Error> {
    for &node in nodes {
        match node {
...
fn resolve_with_env<'cx>(env: &mut ImportEnv<'cx>, parsed: Parsed) -> Result<Resolved<'cx>, Error> {
    let Parsed(expr, base_location) = parsed;
    let mut nodes = Vec::new();
    // First we collect all imports.
    let resolved = traverse_accumulate(env, &mut NameEnv::new(), &mut nodes, &base_location, &expr);
...
/// typechecking.
pub fn resolve<'cx>(cx: Ctxt<'cx>, parsed: Parsed) -> Result<Resolved<'cx>, Error> {
    parsed.resolve_with_env(&mut ImportEnv::new(cx))
}

/// Resolves names, and errors if we find any imports.
pub fn skip_resolve<'cx>(cx: Ctxt<'cx>, parsed: Parsed) -> Result<Resolved<'cx>, Error> {
    let parsed = Parsed::from_expr_without_imports(parsed.0);
    resolve(cx, parsed)
}

impl Parsed {
    fn resolve_with_env<'cx>(self, env: &mut ImportEnv<'cx>) -> Result<Resolved<'cx>, Error> {
...
pub(crate) fn resolve_home(path: impl AsRef<Path>) -> Result<PathBuf, Error> {
    ...
    match path.as_ref().strip_prefix("~") {
        Ok(rest) => {
            let home = home::home_dir().ok_or_else(|| Error::from(ImportError::MissingHome))?;
            f.push(home);
            f.push(rest);
        }
        Err(_) => {
...
impl<SE: Copy> Canonicalize for ImportTarget<SE> {
    fn canonicalize(&self) -> ImportTarget<SE> {
        match self {
            ImportTarget::Local(prefix, file) => ImportTarget::Local(*prefix, file.canonicalize()),
