...
impl Cache {
    ...
    pub fn get<'cx>(&self, cx: Ctxt<'cx>, hash: &Hash) -> Result<Typed<'cx>, Error> {
    ...
    pub fn insert<'cx>(&self, cx: Ctxt<'cx>, hash: &Hash, expr: &Typed<'cx>) -> Result<(), Error> {
        let path = self.entry_path(hash);
...
fn read_cache_file<'cx>(cx: Ctxt<'cx>, path: &Path, hash: &Hash) -> Result<Typed<'cx>, Error> {
...
fn write_cache_file<'cx>(cx: Ctxt<'cx>, path: &Path, expr: &Typed<'cx>) -> Result<(), Error> {
...
mod test {
    ...
    fn filename_for_hash_should_work() {
        let hash = Hash::SHA256(parse_expr("1").unwrap().sha256_hash().unwrap());
        assert_eq!(
            "1220d60d8415e36e86dae7f42933d3b0c4fe3ca238f057fba206c7e9fbf5d784fe15".to_string(),
