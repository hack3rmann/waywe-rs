...
pub fn apply_any<'cx>(f: &Nir<'cx>, a: Nir<'cx>) -> NirKind<'cx> {
    match f.kind() {
        ...
        NirKind::UnionConstructor(l, kts) => NirKind::UnionLit(l.clone(), a, kts.clone()),
...
pub fn merge_maps<K, V, F>(map1: &HashMap<K, V>, map2: &HashMap<K, V>, mut f: F) -> HashMap<K, V>
where
    F: FnMut(&K, &V, &V) -> V,
...
pub fn normalize_one_layer<'cx>(expr: ExprKind<Nir<'cx>>) -> NirKind<'cx> {
    use NirKind::{Assert, Const, NEListLit, NEOptionalLit, Num, RecordLit, RecordType, UnionType};
    ...
    match expr {
        ...
        ExprKind::NEListLit(elts) => ret_kind(NEListLit(elts.into_iter().collect())),
        ExprKind::RecordLit(kvs) => ret_kind(RecordLit(kvs.into_iter().collect())),
        ExprKind::RecordType(kvs) => ret_kind(RecordType(kvs.into_iter().collect())),
        ExprKind::UnionType(kvs) => ret_kind(UnionType(kvs.into_iter().collect())),
...
pub fn normalize_hir<'cx>(env: &NzEnv<'cx>, hir: &Hir<'cx>) -> NirKind<'cx> {
    match hir.kind() {
        ...
        HirKind::Expr(ExprKind::Builtin(b)) => NirKind::from_builtin_env(*b, env.clone()),
        HirKind::Expr(e) => {
