...
impl<'cx> Nir<'cx> {
    ...
    pub fn to_hir(&self, venv: VarEnv) -> Hir<'cx> {
        let map_uniontype = |kts: &HashMap<Label, Option<Nir<'cx>>>| -> ExprKind<Hir<'cx>> {
            ExprKind::UnionType(
                kts.iter()
        ...
        let builtin = |b| Hir::new(HirKind::Expr(ExprKind::Builtin(b)), Span::Artificial);
        ...
        let hir = match self.kind() {
            ...
            self_kind => HirKind::Expr(match self_kind {
                ...
                NirKind::LamClosure {
                    ...
                    annot,
                    closure,
                } => ExprKind::Lam(binder.to_label(), annot.to_hir(venv), closure.to_hir(venv)),
                NirKind::PiClosure {
                    binder,
                    annot,
                    closure,
                } => ExprKind::Pi(binder.to_label(), annot.to_hir(venv), closure.to_hir(venv)),
                ...
                NirKind::OptionalType(t) => {
                    ExprKind::Op(OpKind::App(builtin(Builtin::Optional), t.to_hir(venv)))
                }
                NirKind::EmptyOptionalLit(n) => {
                    ExprKind::Op(OpKind::App(builtin(Builtin::OptionalNone), n.to_hir(venv)))
                }
                ...
                NirKind::TextLit(elts) => {
                    ExprKind::TextLit(elts.iter().map(|t| t.map_ref(|v| v.to_hir(venv))).collect())
                }
                ...
                NirKind::UnionConstructor(l, kts) => ExprKind::Op(OpKind::Field(
                    Hir::new(HirKind::Expr(map_uniontype(kts)), Span::Artificial),
                    l.clone(),
                )),
...
impl<'cx> Closure<'cx> {
    ...
    pub fn apply(&self, val: Nir<'cx>) -> Nir<'cx> {
        match self {
            Closure::Closure { env, body, .. } => body.eval(env.insert_value(val, ())),
            Closure::ConstantClosure { body, .. } => body.clone(),
        }
    }
    fn apply_var(&self, var: NzVar) -> Nir<'cx> {
        match self {
            Closure::Closure { .. } => self.apply(Nir::from_kind(NirKind::Var(var))),
            Closure::ConstantClosure { body, .. } => body.clone(),
        }
    }
...
impl<'cx> TextLit<'cx> {
    pub fn new(elts: impl Iterator<Item = InterpolatedTextContents<Nir<'cx>>>) -> Self {
    ...
    pub fn iter(&self) -> impl Iterator<Item = &InterpolatedTextContents<Nir<'cx>>> {
        self.0.iter()
    }
}
...
impl<'cx> std::cmp::PartialEq for Thunk<'cx> {
    fn eq(&self, _other: &Self) -> bool {
        unreachable!("Trying to compare thunks but we should only compare WHNFs")
    }
}
