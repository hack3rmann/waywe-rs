...
use crate::semantics::resolve::{ImportLocation, download_http_text};
use crate::syntax::{binary, parse_expr};

pub fn parse_file(f: &Path) -> Result<Parsed, Error> {
    let path = crate::resolve::resolve_home(f)?;
    let text = std::fs::read_to_string(path)?;
    let expr = parse_expr(&text).map_err(Box::new)?;
...
pub fn parse_remote(url: Url) -> Result<Parsed, Error> {
    ...
    let expr = parse_expr(&body).map_err(Box::new)?;
    let root = ImportLocation::remote_dhall_code(url);
    Ok(Parsed(expr, root))
}

pub fn parse_str(s: &str) -> Result<Parsed, Error> {
    let expr = parse_expr(s).map_err(Box::new)?;
