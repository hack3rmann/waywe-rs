...
use crate::syntax::{
    self, Expr, ExprKind, FilePrefix, Hash, ImportMode, ImportTarget, InterpolatedTextContents,
    Label, NaiveDouble, Scheme, V,
};
...
impl minicbor::Encode<()> for Expr {
    fn encode<W: minicbor::encode::Write>(
        ...
        match self.as_ref() {
            ...
            Var(V(l, n)) if l.as_ref() == "_" => (*n as u64).encode(enc, ctx)?,
            Var(V(l, n)) => {
                (l, *n as u64).encode(enc, ctx)?;
            }
            Lam(l, x, y) if l.as_ref() == "_" => (1u64, x, y).encode(enc, ctx)?,
            Lam(l, x, y) => (1u64, l, x, y).encode(enc, ctx)?,
            Pi(l, x, y) if l.as_ref() == "_" => (2u64, x, y).encode(enc, ctx)?,
            ...
            EmptyListLit(x) => match x.as_ref() {
                Op(App(f, a)) if matches!(f.as_ref(), ExprKind::Builtin(self::Builtin::List)) => {
