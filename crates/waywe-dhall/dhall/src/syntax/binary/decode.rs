...
use crate::syntax;
use crate::syntax::{
    Expr, ExprKind, FilePath, FilePrefix, Hash, ImportMode, ImportTarget, Integer,
    InterpolatedText, Label, Natural, NumKind, Scheme, Span, URL, UnspannedExpr, V,
};
...
impl<'b> minicbor::Decode<'b, ()> for Value {
    fn decode(
        ...
        Ok(match d.datatype()? {
            ...
            Type::Map | Type::MapIndef => Value::Object(d.map_iter()?.collect::<Result<_, _>>()?),
            ...
            t @ (Type::Undefined | Type::Simple | Type::Int | Type::Break | Type::Unknown(_)) => {
                throw!("Unknown cbor type: {t}")
            }
        })
    }
}
    ...
    Ok(rc(match data {
        String(s) => match Builtin::parse(s) {
            ...
            None => match s.as_str() {
                ...
                _ => return Err(DecodeError::WrongFormatError("builtin".to_owned())),
            },
        },
        ...
        Array(vec) => match vec.as_slice() {
            ...
            [U64(3), U64(n), x, y] => {
                ...
                let op = match n {
                    ...
                    11 => ImportAlt,
                    12 => Equivalence,
                    _ => return Err(DecodeError::WrongFormatError("binop".to_owned())),
                };
                Op(BinOp(op, x, y))
            }
            [U64(4), t] => {
            ...
            [U64(10), x, rest @ ..] => {
                ...
                let labels = rest
                    ...
                    .map(|s| match s {
                        ...
                        _ => Err(DecodeError::WrongFormatError("projection".to_owned())),
                    })
                    .collect::<Result<_, _>>()?;
            ...
            [U64(18), String(first), rest @ ..] => TextLit(InterpolatedText::from((
                first.clone(),
                rest.iter()
                    .tuples()
                    .map(|(x, y)| {
                ...
                let location = match scheme {
                    ...
                    6 => {
                        let env = match rest.next() {
                            Some(String(s)) => s.to_owned(),
                            _ => {
                                return Err(DecodeError::WrongFormatError("import/env".to_owned()));
                            }
                        };
                        ImportTarget::Env(env)
                    }
                    7 => ImportTarget::Missing,
                    _ => return Err(DecodeError::WrongFormatError("import/type".to_owned())),
                };
            ...
            [U64(25), bindings @ ..] => {
                ...
                let bindings = (&mut tuples)
                    .map(|(x, t, v)| {
                        let x = x
                            .as_string()
                            .ok_or_else(|| DecodeError::WrongFormatError("let/label".to_owned()))?;
            ...
            [U64(29), x, labels, y] => {
                ...
                let labels = match labels {
                    Array(labels) => labels
                        ...
                        .map(|s| match s {
                            String(s) => Ok(Label::from(s.as_str())),
                            _ => Err(DecodeError::WrongFormatError("with".to_owned())),
                        })
                        .collect::<Result<_, _>>()?,
                    _ => return Err(DecodeError::WrongFormatError("with".to_owned())),
                };
                Op(With(x, labels, y))
            }
