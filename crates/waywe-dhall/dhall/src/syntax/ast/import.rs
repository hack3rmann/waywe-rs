...
impl<SE> URL<SE> {
    ...
    pub fn map_ref<'a, SE2>(&'a self, f: impl FnOnce(&'a SE) -> SE2) -> URL<SE2> {
...
impl<SE> ImportTarget<SE> {
    ...
    pub fn map_ref<'a, SE2>(&'a self, f: impl FnOnce(&'a SE) -> SE2) -> ImportTarget<SE2> {
        trivial_result(self.traverse_ref(|x| Ok(f(x))))
    }
}
...
impl<SE> Import<SE> {
    ...
    pub fn map_ref<'a, SE2>(&'a self, f: impl FnOnce(&'a SE) -> SE2) -> Import<SE2> {
        trivial_result(self.traverse_ref(|x| Ok(f(x))))
    }
}
