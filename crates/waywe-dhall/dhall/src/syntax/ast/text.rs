...
impl<SubExpr> From<(String, Vec<(SubExpr, String)>)> for InterpolatedText<SubExpr> {
...
impl<SubExpr> InterpolatedTextContents<SubExpr> {
    ...
    pub fn map_ref<'a, SubExpr2, F>(&'a self, mut f: F) -> InterpolatedTextContents<SubExpr2>
    where
...
impl<SubExpr> InterpolatedText<SubExpr> {
    ...
    pub fn iter<'a>(&'a self) -> impl Iterator<Item = InterpolatedTextContents<&'a SubExpr>> + 'a {
        use std::iter::once;
        use InterpolatedTextContents::{Expr, Text};
    ...
    pub fn into_iter(self) -> impl Iterator<Item = InterpolatedTextContents<SubExpr>> {
        use std::iter::once;
        use InterpolatedTextContents::{Expr, Text};
...
impl<SubExpr> FromIterator<InterpolatedTextContents<SubExpr>> for InterpolatedText<SubExpr> {
