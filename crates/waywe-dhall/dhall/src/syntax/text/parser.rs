...
use crate::syntax::{
    Double, Expr, FilePath, FilePrefix, Hash, ImportMode, ImportTarget, Integer, InterpolatedText,
    InterpolatedTextContents, Label, NaiveDouble, Natural, Scheme, Span, URL, UnspannedExpr, V,
};
...
fn insert_recordlit_entry(map: &mut BTreeMap<Label, Expr>, l: Label, e: Expr) {
    ...
    match map.entry(l) {
        ...
        Entry::Occupied(mut entry) => {
            ...
            let span = Span::DuplicateRecordFieldsSugar(Box::new(other.span()), Box::new(e.span()));
            entry.insert(Expr::new(Op(BinOp(RecursiveRecordMerge, other, e)), span));
        }
    }
}
...
impl DhallParser {
    ...
    fn double_quote_chunk(input: ParseInput) -> ParseResult<ParsedTextContents> {
        Ok(match_nodes!(input.into_children();
            [expression(e)] => {
    ...
    fn double_quote_escaped(input: ParseInput) -> ParseResult<String> {
        Ok(match input.as_str() {
            ...
            s => {
                ...
                if s.len() > 8 {
                    return Err(
                        input.error("Escape sequences can't have more than 8 chars".to_string())
                    );
                }
                ...
                match i {
                    0xD800..=0xDFFF => {
                        return Err(input
                            .error("Escape sequences can't contain surrogate pairs".to_string()));
                    }
                    0x0FFFE..=0x0FFFF
                    | 0x1FFFE..=0x1FFFF
                    | 0x2FFFE..=0x2FFFF
                    ...
                    | 0x10_FFFE..=0x10_FFFF => {
                        return Err(input
                            .error("Escape sequences can't contain non-characters".to_string()));
                    }
                    _ => {}
                }
    ...
    fn escaped_quote_pair(_input: ParseInput<'_>) -> ParseResult<&str> {
        Ok("''")
    }
    #[alias(single_quote_char)]
    fn escaped_interpolation(_input: ParseInput<'_>) -> ParseResult<&str> {
        Ok("${")
    }

    // Returns a vec of lines in reversed order, where each line is also in reversed order.
    fn single_quote_continue(input: ParseInput) -> ParseResult<Vec<Vec<ParsedTextContents>>> {
        Ok(match_nodes!(input.into_children();
    ...
    fn builtin(input: ParseInput) -> ParseResult<Expr> {
        ...
        let e = match crate::builtins::Builtin::parse(s) {
            ...
            None => match s {
                ...
                _ => return Err(input.error(format!("Unrecognized builtin: '{}'", s))),
            },
        };
        Ok(spanned(input, e))
    }
    ...
    fn numeric_double_literal(input: ParseInput) -> ParseResult<Double> {
        ...
        match s.parse::<f64>() {
            Ok(x) if x.is_infinite() => {
                Err(input.error(format!("Overflow while parsing double literal '{}'", s)))
            }
    ...
    fn natural_literal(input: ParseInput) -> ParseResult<Natural> {
        ...
        if s.starts_with("0x") {
            ...
            u64::from_str_radix(without_prefix, 16).map_err(|e| input.error(format!("{}", e)))
        } else {
    ...
    fn integer_literal(input: ParseInput) -> ParseResult<Integer> {
        ...
        if rest.starts_with("0x") {
            let without_prefix = sign.to_owned() + rest.trim_start_matches("0x");
            i64::from_str_radix(&without_prefix, 16).map_err(|e| input.error(format!("{}", e)))
        } else {
            s.parse().map_err(|e| input.error(format!("{}", e)))
        }
    }
    ...
    fn quoted_path_component(input: ParseInput) -> ParseResult<String> {
        ...
        Ok(input
            ...
            .map(|c| {
                ...
                if c.is_ascii() {
                    percent_encoding::utf8_percent_encode(&c.to_string(), RESERVED).to_string()
                } else {
                    c.to_string()
                }
            })
    ...
    fn posix_environment_variable_character(input: ParseInput<'_>) -> ParseResult<&str> {
        Ok(match input.as_str() {
            "\\\"" => "\"",
            "\\\\" => "\\",
            "\\a" => "\u{0007}",
    ...
    fn hash(input: ParseInput) -> ParseResult<Hash> {
        ...
        let hash = &s[7..];
        if protocol != "sha256" {
            return Err(input.error(format!("Unknown hashing protocol '{}'", protocol)));
        }
        Ok(Hash::SHA256(hex::decode(hash).unwrap().into()))
    }

    fn import_hashed(input: ParseInput) -> ParseResult<crate::syntax::Import<Expr>> {
    ...
    fn let_binding(input: ParseInput) -> ParseResult<(Label, Option<Expr>, Expr, Span)> {
        Ok(match_nodes!(input.children();
    ...
    fn non_empty_record_type(input: ParseInput) -> ParseResult<BTreeMap<Label, Expr>> {
        Ok(match_nodes!(input.children();
    ...
    fn non_empty_record_literal(input: ParseInput) -> ParseResult<BTreeMap<Label, Expr>> {
        Ok(match_nodes!(input.into_children();
    ...
    fn union_type_entry(input: ParseInput) -> ParseResult<(Label, Option<Expr>)> {
        Ok(match_nodes!(input.children();
...
pub fn parse_expr(input_str: &str) -> ParseResult<Expr> {
    ...
    let inputs = DhallParser::parse_with_userdata(Rule::final_expression, input_str, rc_input_str)?;
