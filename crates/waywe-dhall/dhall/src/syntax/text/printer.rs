...
impl UnspannedExpr {
    ...
    fn annotate_with_phases(&self) -> ExprKind<PhasedExpr<'_>> {
        ...
        match with_base {
            ...
            Op(ProjectionByExpr(a, b)) => Op(ProjectionByExpr(a.phase(Primitive), b)),
            Op(Completion(a, b)) => Op(Completion(a.phase(Primitive), b.phase(Primitive))),
            ExprKind::Import(a) => ExprKind::Import(a.map_ref(|x| x.phase(PrintPhase::Import))),
            e => e,
        }
    }

    fn fmt_phase(&self, f: &mut fmt::Formatter, phase: PrintPhase) -> Result<(), fmt::Error> {
        use ExprKind::*;
        use OpKind::*;
...
fn fmt_label(label: &Label, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
    ...
    let is_reserved = match s.as_str() {
        "let" | "in" | "if" | "then" | "else" | "Type" | "Kind" | "Sort" | "True" | "False"
        | "Some" => true,
        _ => Builtin::parse(&s).is_some(),
    };
    if s.is_empty() {
        write!(f, "``")
    } else if !is_reserved && s.chars().all(|c| c.is_ascii_alphanumeric() || c == '_') {
        write!(f, "{}", s)
    } else {
        write!(f, "`{}`", s)
    }
                    ...
                    for c in a.chars() {
                        match c {
                            ...
                            '\u{0000}'..='\u{001F}' => {
                                ...
                                // Print as "\uXXXX"
                                write!(f, "\\u{:0>4}", &escaped[3..escaped.len() - 1])
                            }
                            c => write!(f, "{}", c),
                        }?;
                    }
