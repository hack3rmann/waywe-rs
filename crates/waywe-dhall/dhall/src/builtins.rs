...
impl<'cx> BuiltinClosure<'cx> {
    ...
    pub fn to_hirkind(&self, venv: VarEnv) -> HirKind<'cx> {
        HirKind::Expr(self.args.iter().fold(ExprKind::Builtin(self.b), |acc, v| {
...
fn apply_builtin<'cx>(b: Builtin, args: Vec<Nir<'cx>>, env: NzEnv<'cx>) -> NirKind<'cx> {
    let cx = env.cx();
    use NirKind::*;
                ...
                Some(n) => {
                    ...
                    if let Some(h) = nir_to_string(haystack) {
                        ...
                        if let Some(r) = nir_to_string(replacement) {
                            ...
                            let parts = h
                                .split(&n)
                                .map(|s| InterpolatedTextContents::Text(s.to_string()));
                            let replacement = InterpolatedTextContents::Expr(replacement.clone());

                            Ret::Nir(Nir::from_kind(NirKind::TextLit(nze::nir::TextLit::new(
                                Itertools::intersperse(parts, replacement),
                            ))))
                        }
                    } else {
                        Ret::DoneAsIs
        ...
        (Builtin::ListIndexed, [t, l]) => {
            match l.kind() {
                EmptyListLit(_) | NEListLit(_) => {
                    ...
                    kts.insert("index".into(), Nir::from_builtin(cx, Builtin::Natural));
