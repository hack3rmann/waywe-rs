...
impl TestFile {
    pub fn path(&self) -> PathBuf {
        match self {
            TestFile::Source(path) | TestFile::Binary(path) | TestFile::UI(path) => {
                PathBuf::from("dhall").join(path)
            }
        }
    }
    ...
    pub fn compare_binary(&self, expr: Expr) -> Result<()> {
        match self {
            TestFile::Binary(_) => {}
            _ => return Err(TestError("This is not a binary file".to_string()).into()),
        }
        if !self.path().is_file() {
            return self.write_expr(expr);
        }
...
fn discover_tests_for_feature(feature: TestFeature) -> Vec<Trial> {
    let take_ab_suffix = feature.output_type != FileType::UI || feature.module_name == "printer";
    ...
    for base_path in TEST_PATHS {
        ...
        for path in dhall_files_in_dir(&tests_dir, take_ab_suffix, feature.input_type) {
            ...
            let name = path.replace("\\", "_").replace("/", "_").replace("-", "_");
