...
impl<'de: 'a, 'a> serde::Deserializer<'de> for Deserializer<'a> {
        ...
        V: serde::de::Visitor<'de>,
    {
        ...
        match self.0.as_ref() {
            ...
            List(xs) => visitor.visit_seq(SeqDeserializer::new(xs.iter().map(val))),
            ...
            Union(field_name, Some(x)) => visitor.visit_enum(MapAccessDeserializer::new(
                MapDeserializer::new(Some((field_name.as_str(), val(x))).into_iter()),
            )),
            Union(field_name, None) => visitor.visit_enum(MapAccessDeserializer::new(
                MapDeserializer::new(Some((field_name.as_str(), ())).into_iter()),
            )),
        }
    }

    fn deserialize_tuple<V>(self, _: usize, visitor: V) -> crate::Result<V::Value>
    where
        V: serde::de::Visitor<'de>,
    {
        let val = |x| Deserializer(Cow::Borrowed(x));
