...
impl<'a> Deserializer<'a, NoAnnot> {
    ...
    pub fn type_annotation<'ty>(self, ty: &'ty SimpleType) -> Deserializer<'a, ManualAnnot<'ty>> {
        Deserializer {
...
impl<'a, A> Deserializer<'a, A> {
    ...
    pub fn with_builtin_types(
        ...
        self.builtins.extend(
            tys.into_iter()
                .map(|(s, ty)| (dhall::syntax::Label::from_str(&s), ty.to_expr())),
        );
        self
    }
        ...
        T: HasAnnot<A>,
    {
        Ctxt::with_new(|cx| {
            ...
            let parsed_with_builtins = self.builtins.iter().fold(parsed, |acc, (name, subst)| {
...
///
/// [`parse()`]: Deserializer::parse()
pub fn from_binary_file<'a, P: AsRef<Path>>(path: P) -> Deserializer<'a, NoAnnot> {
