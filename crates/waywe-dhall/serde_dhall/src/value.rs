...
impl Value {
    pub(crate) fn from_nir_and_ty<'cx>(cx: Ctxt<'cx>, x: &Nir<'cx>, ty: &Nir<'cx>) -> Result<Self> {
...
impl SimpleValue {
    fn from_nir(nir: &Nir) -> StdResult<Self, NotSimpleValue> {
        Ok(match nir.kind() {
            ...
            NirKind::UnionLit(field, x, _) => {
                SimpleValue::Union(field.into(), Some(Box::new(Self::from_nir(x)?)))
            }
    ...
    fn to_hir<'cx>(&self, ty: Option<&SimpleType>) -> Result<Hir<'cx>> {
        ...
        let kind = match (self, ty) {
            ...
            (V::Text(v), Some(T::Text)) | (V::Text(v), None) => ExprKind::TextLit(v.clone().into()),

            (V::Optional(None), None) => return Err(type_missing()),
            (V::Optional(None), Some(T::Optional(t))) => ExprKind::Op(OpKind::App(
                hir(ExprKind::Builtin(Builtin::OptionalNone)),
                t.to_hir(),
            )),
            (V::Optional(Some(v)), None) => ExprKind::SomeLit(v.to_hir(None)?),
            (V::Optional(Some(v)), Some(T::Optional(t))) => ExprKind::SomeLit(v.to_hir(Some(t))?),
            ...
            (V::List(v), None) => {
                ExprKind::NEListLit(v.iter().map(|v| v.to_hir(None)).collect::<Result<_>>()?)
            }
            (V::List(v), Some(T::List(t))) => {
                ExprKind::NEListLit(v.iter().map(|v| v.to_hir(Some(t))).collect::<Result<_>>()?)
            }
            ...
            (V::Union(variant, Some(v)), Some(T::Union(t))) => match t.get(variant) {
            ...
            (V::Union(variant, None), Some(T::Union(t))) => match t.get(variant) {
                Some(None) => {
                    ExprKind::Op(OpKind::Field(ty.unwrap().to_hir(), variant.clone().into()))
                }
                _ => return Err(type_error()),
            },
    ...
    pub(crate) fn to_expr(&self, ty: Option<&SimpleType>) -> Result<Expr> {
        Ctxt::with_new(|cx| Ok(self.to_hir(ty)?.to_expr(cx, Default::default())))
    }
}
...
impl SimpleType {
    fn from_nir(nir: &Nir) -> StdResult<Self, NotSimpleType> {
        Ok(match nir.kind() {
            ...
            NirKind::OptionalType(t) => SimpleType::Optional(Box::new(Self::from_nir(t)?)),
            NirKind::ListType(t) => SimpleType::List(Box::new(Self::from_nir(t)?)),
            ...
            NirKind::UnionType(kts) => SimpleType::Union(
                kts.iter()
                    .map(|(k, v)| Ok((k.into(), v.as_ref().map(Self::from_nir).transpose()?)))
                    .collect::<StdResult<_, _>>()?,
            ),
    ...
    pub(crate) fn to_hir<'cx>(&self) -> Hir<'cx> {
        ...
        hir(match self {
            ...
            SimpleType::Union(kts) => ExprKind::UnionType(
                kts.iter()
                    .map(|(k, t)| (k.as_str().into(), t.as_ref().map(|t| t.to_hir())))
                    .collect(),
            ),
        })
    }
